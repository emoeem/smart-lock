...
from src.core.auth import build_auth_provider_from_config, VoiceAuthProvider
...
# /home/emo/environment/voice/.venv/bin/python \
#   /home/emo/environment/voice/cam++_model/smart_lock_system/speaker_verification.py
...
class SmartLockSystem:
    def __init__(self):
        ...
        try:
            # 绑定语音输出（如果有）以便主进程显示语音模块输出
            self._bind_voice_output(self.auth_provider)
        except Exception:
            log.debug("绑定语音输出回调时发生异常")
            ...
            if "command/open" in topic:
                ...
                try:
                    ...
                    if updated:
                        ...
                        # 如果本次远程更新包含 lock_features 的修改，则需要重启程序以使功能集生效
                        try:
                            restart_required = False
                            # 情况 A: 点路径更新
                            if isinstance(data, dict) and data.get('key'):
                                key = str(data.get('key'))
                                if key.endswith('lock_features'):
                                    restart_required = True
                            # 情况 B: 字典合并形式
                            if isinstance(data, dict) and data.get('lock_config'):
                                lc = data.get('lock_config')
                                if isinstance(lc, dict) and 'lock_features' in lc:
                                    restart_required = True
                            if restart_required:
                                log.info("检测到 lock_features 更新，需重启以应用更改，请手动重启程序。")
                                import os
                                import subprocess
                                RUN_SH = "/home/emo/smart_lock/run.sh"
                                RESTART_DELAY = 10  # 秒
                                try:
                                    subprocess.Popen(
                                        f"(sleep {RESTART_DELAY} && bash {RUN_SH}) &",
                                        shell=True,
                                        stdout=subprocess.DEVNULL,
                                        stderr=subprocess.DEVNULL
                                    )
                                    print(f"已启动后台重启任务，{RESTART_DELAY}s 后执行 run.sh")
                                    os._exit(0)  # 使用 os._exit 强制退出，不会触发异常处理
                                except Exception as e:
                                    print(f"启动后台重启任务失败: {e}")

                        except Exception:
                            log.error("检查 lock_features 更新时发生异常")
                    else:
    ...
    def _on_plus_key(self):
        """处理 USB 小键盘 '+' 按键：触发语音验证流程（记录日志并调用 VoiceAuthProvider）。"""
        try:
            log.info("检测到 '+' 按键，开始语音验证触发")
            def invoke(provider):
                """递归调用 provider，返回 True 表示认证成功（已开门）。"""
                if provider is None:
                    return False
                # 复合提供器处理：任何子项成功则返回 True
                ...
                # 单一提供器：检测是否为 VoiceAuthProvider
                try:
                    if isinstance(provider, VoiceAuthProvider):
                        try:
                            ok = provider.authenticate(None)
                        except Exception:
                            log.debug("调用 VoiceAuthProvider.authenticate 失败")
                            ok = False
                        if ok:
                            try:
                                self.lock.open_door(reason="voice")
                                log.info("语音验证成功，已执行开门")
                            except Exception:
                                log.debug("调用 open_door 失败")
                            return True
                except Exception:
                    # 回退的类名检查
                    if getattr(getattr(provider, '__class__', None), '__name__', '') == 'VoiceAuthProvider':
                        try:
                            ok = provider.authenticate(None)
                        except Exception:
                            ok = False
            ...
            invoked = invoke(self.auth_provider)
            if not invoked:
                log.info("语音验证未通过或未找到 VoiceAuthProvider")
        except Exception as e:
            log.debug(f"处理 '+' 按键触发语音验证失败: {e}")

    def _bind_voice_output(self, provider):
        """递归查找 VoiceAuthProvider 并绑定输出回调，将输出打印并推送到 Web UI。"""
        if provider is None:
            return
        if hasattr(provider, 'providers'):
            for p in getattr(provider, 'providers'):
                self._bind_voice_output(p)
            return

        try:
            # 绑定输出回调
            if getattr(provider, '__class__', None) and getattr(provider.__class__, '__name__', '') == 'VoiceAuthProvider':
                try:
                    # 将语音模块的输出封装为带 type 字段的 JSON，避免前端将其当作密码填充
                    provider.set_on_output(lambda text, is_err: send_keyboard_to_clients(json.dumps({
                        'type': 'voice',
                        'message': text,
                        'is_error': bool(is_err)
                    })))
                except Exception:
                    log.debug("绑定 VoiceAuthProvider 输出回调失败")
        except Exception:
            pass
